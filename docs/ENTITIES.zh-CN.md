...
| Fine rice taste（精煮口感） | enum sensor | ✓ | ✓ | 仅精煮饭烹饪或预约中显示设备反馈的 `soft/middle/hard`；其他菜单、保温、待机及缺失反馈均为 `unknown`。CMC301 来自 texture 属性，normal3 来自 stage 中的口感字段。 |
...
核心状态、菜单、时长、倒计时在一次请求后先发布；口感等辅助属性、温度历史和设置随后在后台依次补齐。设备请求继续串行，每项辅助读取不重试，并在请求之间释放设备锁。新轮询、控制命令和卸载会取消旧后台任务；已开始的底层请求可能仍需等到响应或超时，但其迟到结果不会覆盖新状态，也不会让断线实体恢复可用。

设置最多缓存 120 秒，HA 内修改设置后以读回值立即更新缓存。温度历史保留原有按状态的 30/120 秒间隔，状态或食谱变化时先清空旧历史。常规轮询仍为 30 秒，命令后立即刷新核心状态、2 秒后补刷，再加载辅助信息。辅助结果不会重置常规轮询计时。
